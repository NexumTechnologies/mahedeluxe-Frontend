  ...
  return (
    <div className="space-y-6" dir={dir}>
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-gray-900">{td("adminSellerApprovals.title")}</h1>
          <p className="text-sm text-gray-500 mt-1">{td("adminSellerApprovals.subtitle")}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="hidden sm:block text-xs text-gray-500">
            <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-3 py-1 ring-1 ring-gray-100">
              ...
              {td("adminSellerApprovals.totalPending", { total })}
            </span>
          </div>
          <div>
            <input
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
        {isLoading ? (
          <div className="p-6 text-center text-sm text-gray-500">{td("adminSellerApprovals.loading")}</div>
        ) : error ? (
          <div className="p-6 text-center text-sm text-red-600">{td("adminSellerApprovals.failed")}</div>
        ) : (
          <>
            ...
            <div className="p-4 bg-gray-50/60">
              {users.length === 0 ? (
                <div className="py-8 text-center text-sm text-gray-500">{td("adminSellerApprovals.empty")}</div>
              ) : (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex flex-wrap gap-2 text-xs text-gray-500">
                            <span>
                              {td("adminSellerApprovals.products")} <span className="font-medium text-gray-700">{s.productsCount ?? 0}</span>
                            </span>
                            <span className="inline-block h-1 w-1 rounded-full bg-gray-300" />
                            <span>
                              {td("adminSellerApprovals.ordersReceived")} <span className="font-medium text-gray-700">{s.ordersReceived ?? 0}</span>
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        >
                          ...
                          {approveMutation.isPending && pendingUserId === s.id ? td("common.approving") : td("common.approve")}
                        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-lg rounded-2xl bg-white shadow-xl">
            <div className="border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-slate-900">Reject Seller</h2>
