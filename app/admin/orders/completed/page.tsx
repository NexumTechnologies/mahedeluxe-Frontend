        ...
        <div className="app-modal-scroll p-5 space-y-4">
          <div className="flex gap-4">
            ...
            <div className="flex-1 space-y-1">
              ...
              <div className="text-xs text-gray-500">
                Delivered at: <span className="font-medium text-gray-800">{formatDate(order.updatedAt ?? order.createdAt)}</span>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6 relative">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-gray-900">Completed Orders</h1>
          <p className="text-sm text-gray-500 mt-1">
            Orders that have been delivered successfully to buyers.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
        {isLoading ? (
          <div className="p-6 text-center text-sm text-gray-500">Loading completed orders...</div>
        ) : error ? (
          <div className="p-6 text-center text-sm text-red-600">Failed to load completed orders</div>
        ) : (
          <>
            ...
            <div className="p-4 bg-gray-50/60">
              {orders.length === 0 ? (
                <div className="py-8 text-center text-sm text-gray-500">No completed orders.</div>
              ) : (
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span
                                className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ${STATUS_BADGE.delivered}`}
                              >
                                <span className="h-1.5 w-1.5 rounded-full bg-current" /> Delivered
                              </span>
                            </div>
                            ...
                            <div className="text-xs text-gray-500 flex flex-wrap gap-x-3 gap-y-1">
                              <span>
                                Delivered at: <span className="font-medium text-gray-700">{formatDate(order.updatedAt ?? order.createdAt)}</span>
                              </span>
